...
class NaiveRNN(BaseModel):
    ...
    def __init__(
        ...
        with tf.variable_scope("output"):
            # For a text classification task, we only need the output of the last rnn element in the text sequence
            rnn_last_element_output = rnn_output[:, -1, :]
            
            logits = add_fully_connected_layer(self, rnn_last_element_output)
